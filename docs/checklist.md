# BDX 개발 체크리스트

> 클라이언트 피드백 기반. 최종 업데이트: 2026-06-04

**상태**: ⬜ 미착수 · 🟡 진행중 · 🔵 부분완료 · 🟢 완료 · 🔴 블로커

---

## 1. 제품 방향 전환 — "상담 시스템 → 고객 CRM"

> 핵심: BDX는 ERP가 아니라 **고객 성향 관리 + 시술 기록 CRM**

### 1.1 ⬜ CLAUDE.md 제품 설명 업데이트
- **지금**: "네일 샵 상담 플로우 앱"으로 소개됨
- **할 일**: "네일 샵 고객 관리 CRM + 시술 기록 시스템"으로 변경. 핵심 기능 3가지(스케줄 CRM, 포트폴리오, 고객 성향 관리) 명시

---

## 2. 홈 화면 — 스케줄 중심으로 개편

> 핵심: 원장님이 앱 열면 **오늘 스케줄 + 고객 특징 태그**가 바로 보여야 함

### 2.1 🟢 홈 화면을 스케줄표 중심으로 변경
- **완료**: 스케줄표 중심 레이아웃 적용됨
- **완료 추가**: 기록 관리 페이지에서 `useShopStore((s) => s.getActiveDesigners())`처럼 새 배열을 반환하던 selector를 제거하고, 원본 `designers` 상태를 `useMemo`로 파생하도록 바꿔 React `getSnapshot` 무한 루프 경고를 해소함
- **완료 추가**: 홈 `오늘의 예약` 카드 내부에 예약 참고 이미지(`referenceImageUrls` / 사전상담 referenceImages)와 기존 고객 포트폴리오 사진을 작은 썸네일 스트립으로 노출하고, 예약 폼 업로드 이미지는 `blob:` 임시 URL 대신 data URL로 저장해 새로고침 후에도 썸네일이 깨지지 않게 함

### 2.2 🟢 예약 카드에 고객 특징 태그 표시
- **완료**: 예약 카드가 `customerId`로 고객의 주요 태그를 조회해 칩 형태로 최대 3개까지 표시함. pinned 태그가 있으면 우선 노출하고, 없으면 기존 고객 태그를 우선순위대로 보여줌
  ```
  김지영 · 자석젤
  [큐티클 민감] [오버레이 선호] [유지력 약함]
  ```
- **참고**: 시술 전 알림 모달은 기존처럼 pinned 태그만 사용하고, 카드 표시용 태그만 별도 우선순위 조회 로직으로 분리함
- **완료 추가**: `records` 일간 디자이너 캘린더의 드래그 예약 카드도 홈 카드처럼 서비스 라벨 + 고객 태그를 카드 내부에 표시하도록 보강하고, 1시간 슬롯 높이를 키워 태그 줄이 잘리지 않게 조정함

### 2.3 🟢 태그 색상 코딩
- **완료**: `CustomerTag.accent` 커스텀 색상을 유지하면서 카테고리별 기본 색상을 자동 적용하도록 공통 태그 컬러 시스템을 반영함. 스케줄 카드, 고객 상세, 예약 연결 모달, 시술 전 알림, 기록 상세에서 동일한 색상 규칙을 사용함

### 2.4 🟢 상담 준비 상태 아이콘
- **완료**: 예약별 사전 상담 완료 여부를 저장하고, 홈/예약 리스트/캘린더/예약 상세에서 `🟢 디자인 확정` / `🟡 현장 상담 필요` 상태 아이콘으로 표시함. 상담 링크 제출이 완료되면 자동으로 디자인 확정 상태로 전환됨

### 2.5 🟢 사전상담 알림 센터 실데이터 연동
- **완료**: 상단/사이드 알림 벨을 실제 사전상담 알림 센터와 연결하고, 미확인 개수를 배지로 표시함. 알림 목록은 예약 실데이터의 `preConsultationCompletedAt` 기준으로 생성되며, 읽음 상태는 예약별로 저장됨
- **완료 추가**: 홈 상단 배너와 알림 센터가 동일한 읽음 상태를 공유하고, 알림 클릭 시 `records?bookingId=`를 통해 해당 예약 상세 바텀시트가 자동으로 열리도록 연결함

### 2.6 🟢 오늘 날짜/집계 기준을 한국 시간으로 통일
- **완료**: 홈/대시보드/기록/캘린더에서 `today` 계산과 날짜 표기를 `Asia/Seoul` 기준으로 통일해, 서버가 UTC여도 한국 기준 오늘 날짜가 어제로 보이던 문제를 수정함
- **완료 추가**: 당일 예약/상담 집계도 KST 날짜 문자열로 비교하도록 맞춰, 자정 전후에도 `오늘` 카드와 KPI 값이 한국 시간 기준으로 일관되게 보이도록 정리함
- **완료 추가**: 예약/상담/고객/포트폴리오/설정 저장 시 생성되는 새 타임스탬프, 포트폴리오 기본 촬영일, mock 상대 날짜, 상대 날짜 계산까지 KST helper로 통일해 전체 흐름에서 날짜 기준이 섞이지 않게 정리함

### 2.7 🟢 홈 예약 카드 `고객 연결 필요` 버튼 좌측 정렬
- **완료**: 홈 `오늘의 예약` 카드에서 고객 미연결 상태일 때 보이는 `고객 연결 필요` 버튼에 `self-start text-left` 정렬을 적용해, 카드 내부에서 가운데로 보이던 CTA가 왼쪽 정보 컬럼 시작선에 맞춰 정렬되도록 수정함
- **완료 추가**: 홈 `오늘의 예약` 카드에서 포트폴리오/참고 썸네일이 생기면 모바일 한 줄 폭이 무너지던 문제를 수정하고, 모바일에서는 `시간 + 본문`과 `썸네일 + 액션`을 두 줄로 분리해 고객명/태그가 세로로 찌그러지지 않게 정리함
- **완료 추가**: 홈 카드 섹션 타이틀을 실제 사용 맥락에 맞춰 `오늘의 예약`에서 `오늘의 상담`으로 변경해, 상단 CTA/기록 흐름과 용어가 자연스럽게 이어지도록 정리함
- **완료 추가**: 홈 통계 카드의 `오늘 예약`도 `상담 예정`으로 맞추고, 온보딩 가이드/인트로 데모에 남아 있던 `오늘의 예약` 문구도 `오늘의 상담` 기준으로 통일해 홈 진입 문맥 전체를 일관되게 정리함
- **완료 추가**: 홈 Hero CTA의 보조 카드 제목도 `새 예약 등록`에서 `새 상담 등록`으로 바꿔, 홈 진입 주요 액션의 용어를 상담 중심 톤으로 맞춤

---

## 3. 포트폴리오 — 시술 사진 검색 시스템

> 핵심: "예전에 다른 손님이 했던 스타일" 요청 시 **사진으로 바로 찾고, 가격까지 확인**

### 3.1 🟢 포트폴리오 갤러리 강화
- **완료**: 포트폴리오 갤러리에 시술 종류/기간/가격 필터를 추가하고, 검색 대상을 고객명에서 태그·컬러·디자인 타입까지 확장함. 업로드 폼도 메타데이터 입력을 지원해 필터링 가능한 구조로 보강됨
- **완료 추가**: 포트폴리오 업로드 폼의 고객 선택 드롭다운에 별도 open/close 상태를 도입해, 고객 선택 또는 직접 추가 직후 결과 목록이 남아 있지 않고 입력창에는 선택된 고객명만 유지되도록 정리함
- **완료 추가**: 포트폴리오 업로드의 `시술 종류` 입력을 자유 텍스트 대신 `원컬러/단색+포인트/풀아트/이달의 아트` 선택형 UI로 정리해 메타데이터 입력값이 더 일관되게 저장되도록 보강함
- **완료 추가**: demo shop(`shop-demo`) 포트폴리오 시드를 `src/data/demo-portfolio.ts`로 단일화하고, customer-link 상담의 `PortfolioBrowser`도 같은 시드를 사용하도록 정리해 메인 포트폴리오와 고객 상담 화면에서 보이는 이미지/메타데이터가 일치하게 맞춤

### 3.2 🟢 사진 상세에서 시술 정보 + 가격 표시
- **완료**: 사진 상세 페이지에서 시술 정보 + 가격 표시 구현됨

### 3.3 🟢 포트폴리오 ↔ 고객 ↔ 상담 데이터 연결
- **완료**: 포트폴리오-고객-상담 간 데이터 연결 구현됨

### 3.4 🟢 대시보드에 인기 시술 사진 썸네일
- **완료**: 대시보드 `인기 시술 Top 3`를 실제 포트폴리오 `시술` 사진 기반 썸네일 카드로 교체하고, 클릭 시 대표 이미지·컬러번호·가격·고객·촬영일을 확인하는 팝업을 연결함. 포트폴리오 사진이 없을 때는 업로드 유도 빈 상태를 표시함

### 3.5 🟢 포트폴리오 기반 상담 시작 prefill 정리
- **완료**: 포트폴리오 오버레이의 `이 디자인으로 상담 시작`이 더 이상 기존 고객 이름/전화번호/고객 ID를 쿼리로 넘기지 않도록 수정함
- **완료 추가**: 선택한 포트폴리오 이미지는 상담 `referenceImages`로만 미리 채우고, 고객 정보는 비워 둔 상태로 `/consultation/customer`에서 새 상담을 시작하게 정리함

---

## 4. 고객 카드 — 성향 태그 관리

> 핵심: 고객 특성을 **기억이 아닌 데이터**로 관리. 강조·색상·고정 가능

### 4.1 🟢 고객 성향 태그 시스템
- **완료**: 고객 성향 태그 시스템 구현됨

### 4.2 🟢 태그 강조/고정 UI
- **완료**: 고객 상세 화면에서 태그별 `강조` 토글, 색상 선택, `상단 고정` 토글, 고정 태그 드래그 정렬 UI를 추가함. 편집 중에는 로컬 미리보기로 상단 특이사항/태그 순서가 즉시 반영되고, 저장 시 홈/예약 카드 우선 노출 순서까지 함께 갱신됨

### 4.3 🟢 VIP 비주얼 카드
- **완료**: VIP 비주얼 카드 구현됨

### 4.4 🟢 시술 이력 타임라인
- **완료**: 시술 이력 타임라인 구현됨

### 4.5 🟢 `VIP` → `단골` 용어 통일
- **완료**: 고객 목록 필터 탭, 통계 카드, 리스트 배지의 `VIP` 문구를 `단골`로 통일함
- **완료 추가**: 고객 상세의 단골 토글 토스트/접근성 라벨도 `단골 지정됨`, `단골 해제` 기준으로 정리해 용어 혼재를 제거함

### 4.6 🟢 고객 목록 검색 + 페이지네이션
- **완료**: 고객 목록 검색 범위를 이름/전화번호/담당자에서 고객 ID와 태그까지 확장하고, 검색 placeholder도 실제 검색 범위에 맞게 갱신함
- **완료 추가**: 고객 목록에 10명 단위 페이지네이션과 `이전/다음` 이동 UI를 추가하고, 검색어나 필터가 바뀌면 자동으로 1페이지로 리셋되게 정리함

### 4.7 🟢 고객 상세 → 상담 기록 탭 연동 강화
- **완료**: 고객 상세의 `상담 기록 보기`가 `/records?customerId={id}&view=list`로 이동하도록 연결하고, 기록 페이지가 해당 쿼리를 읽어 상담 기록 탭으로 자동 전환되게 함
- **완료 추가**: 기록 페이지 상단에 고객 기준 필터 배너와 `필터 해제` 액션을 추가해, 특정 고객 기록만 보는 상태를 명확히 표시함
- **완료 추가**: 고객 상세의 시술 사진 그리드 삭제 버튼이 `group-hover`에만 묶여 있던 문제를 수정해, 태블릿/모바일에서도 X 버튼이 항상 보여 바로 삭제할 수 있고 데스크톱에서는 기존 hover 감도도 유지되게 조정함
- **완료 추가**: 고객 상세의 최근 시술/시술 사진 갤러리에서 빈 `imageDataUrl` 항목을 렌더 전에 제외해, `next/image`가 빈 `src`로 콘솔 에러를 내지 않도록 안전 가드를 추가함
- **완료 추가**: 고객 상세의 `시술 성향 태그` 편집 헤더와 `상단 카드 노출 순서` 안내 영역은 모바일에서 텍스트보다 액션 버튼/배지가 먼저 가로폭을 차지하던 문제를 수정하고, 모바일에서는 설명 아래로 버튼/배지가 자연스럽게 내려오도록 정리해 줄바꿈이 비정상적으로 깨지지 않게 보강함

---

## 5. 시술 전 알림 — 고객 주의사항 미리 확인

> 핵심: 시술 시작 전 "이 고객은 큐티클이 민감합니다" 같은 **팝업 알림**

### 5.1 🟢 시술 전 알림 모달
- **완료**: `상담 시작` 전에 고객 주요 태그를 먼저 확인하는 시술 전 알림 모달을 홈/예약 리스트 흐름에 적용함. 태그별 주의 문구와 우선순위를 함께 표시해 확인 후 상담에 진입할 수 있음

### 5.2 🟢 세이프티 태그 색상 코드
- **완료**: 고객 태그 값을 기반으로 `🔴 높음 / 🟠 중간 / 🟡 참고 / 🟢 선호` 세이프티 레벨을 자동 분류하고, 시술 전 알림/예약 상세에서 동일한 색상과 안내 문구를 사용함

---

## 6. 상담 UX — 3단계로 간소화 (2026-03-27 회의 결정)

> 핵심: 현재 상담이 복잡함. **최대한 빠르게 끝나야** 시술 가능 고객 수가 늘어남
> 참조: `production-docs/회의록-2026-03-27.md`

### 6.1 🟢 상담 흐름 3단계로 재설계 (A-2, A-3, A-10)
- **완료**: STEP_ORDER를 `START → STEP1_BASIC → STEP2_DESIGN → CUSTOMER_INFO → SUMMARY`로 재구성
- **완료**: 캔버스(CANVAS)와 특성 선택(TRAITS)을 메인 플로우에서 제거 (A-10). TRAITS는 별도 접근 가능
- **완료**: 고객 정보 입력을 마지막 단계(step 3)로 이동 (A-3). customer_link에서는 이미 prefill되어 확인만
- **완료**: 모든 step 페이지 totalSteps=3, stepNumber 업데이트
- **완료**: 시작 화면(staff/customer_link) → step1으로 직행. 기존 customer 페이지 경유 제거
- **완료 이전**: 상담 6단계 흐름 구현 → 7단계 → 5단계 → **3단계**로 최종 간소화

### 6.2 🟢 포트폴리오 첫 화면 통합 (A-4)
- **완료**: PortfolioBrowser를 step1에 통합. customer_link 전용 조건 제거 → 모든 entryPoint에서 포트폴리오 + 참고 이미지 업로드가 첫 화면에 노출
- **완료**: MoodTagSelector도 step1으로 이동

### 6.3 🟢 고객 선호도 자동 채우기 (A-9)
- **완료**: summary 저장 시 `consultation.nailShape` → `customer.preference.preferredShape` 자동 매핑. 원장님이 고객 카드에서 수동 입력할 필요 없음

### 6.4 🟢 기존 라우트 → 신규 라우트 마이그레이션
- **완료**: consultation/ 라우트 구조 전환됨

### 6.5 🟢 고객용 상담에서 기존 고객 검색 차단
- **완료**: `customer_link` 진입 시 고객 정보 화면에서 `기존 고객` 검색/선택 토글과 검색 결과 리스트를 숨기고, 신규 정보 입력 폼만 보이도록 정리함
- **완료 추가**: 고객용 상담에서는 기존 고객 선택 callback이 노출되지 않도록 제한해, QR 링크 접속 고객이 원장님용 CRM 검색 흐름에 접근할 수 없게 함

### 6.6 🟢 고객용 상담 화면 분리 및 owner 전용 제어 숨김
- **완료**: 고객용 요약 화면에서 할인 적용, 추가금 입력, 최종 결제 카드, DiscountModal 같은 원장님 전용 제어를 숨기고 제출 중심 화면으로 분리함
- **완료 추가**: 고객용 완료 화면에서도 `고객 상세 보기`, `홈으로 돌아가기` 같은 내부 운영 동선을 제거하고, `새 상담 다시 작성하기` 중심의 고객 전용 완료 경험으로 정리함
- **완료 추가**: staff용 `/consultation/summary` 최종 결제 카드에서 raw i18n key `consultation.basePrice`가 노출되던 문제를 정리하고, 실제 금액 의미에 맞는 `소계` 라벨로 교체함
- **완료 추가**: 고객 상담 링크/QR에 `shopName`을 함께 실어 보내고, 고객 정보/요약/완료 화면 상단에 해당 샵명을 노출해 이 상담 링크가 어느 샵에서 온 것인지 바로 확인할 수 있게 보강함

---

## 7. 가격 입력 — 시술 종료 후 확정

> 핵심: 네일 가격은 시술 중 변동이 많으므로, **시술 끝나고 최종 가격 입력**

### 7.1 🟢 시술 완료 후 가격 입력 화면
- **완료**: 시술 완료 후 가격 입력 화면 구현됨
- **완료 추가**: 시술 확인서에서 할인/예약금도 함께 확정할 수 있게 보강하고, 정액 할인·할인율·예약금을 숫자 프리셋과 +/- 선택 방식으로 조정하도록 바꿔 수기 입력 의존도를 낮춤
- **완료 추가**: 시술 확인서에서 가격 확정 뒤 `저장하기`를 누를 때 상담 스토어 reset이 먼저 실행되며 `/consultation` 가드로 튀던 문제를 수정하고, 이제 `/home` 이동을 먼저 시작한 뒤 상태를 정리하도록 바꿈
- **완료 추가**: record 기반으로 직접 열린 시술 확인서는 consultation entryPoint가 없어도 접근 가능하도록 guard를 완화해, `consultationId`가 유효한 경우 저장 직후 `/consultation`으로 다시 튀지 않고 `/home`으로 안정적으로 복귀하도록 수정함

### 7.2 🟢 가격 히스토리
- **완료**: 가격 히스토리 연동 구현됨

---

## 8. 상담 입력 방식 — 링크 발송 + 태블릿

> 핵심: 상담 데이터를 모으는 **2가지 경로** 제공

### 8.1 🟢 고객 상담 링크 발송
- **완료**: 상담 링크 발송 기능 구현됨
- **완료 추가**: `entry=customer-link` 로 진입한 고객 상담 링크는 이제 `src/app/consultation/customer/page.tsx` 에서 2.5초 브랜드 스플래시를 먼저 보여준 뒤 상담 화면으로 전환됨. 같은 링크 세션에서는 스플래시를 반복 노출하지 않아 뒤로가기/재진입 시 흐름이 끊기지 않음
- **완료 추가**: 고객 상담 링크/QR 기본 진입 경로를 `/consultation` 루트로 통일하고, 기존 `/consultation/customer?entry=customer-link...` 링크로 들어와도 query string을 유지한 채 루트 첫 화면(디자이너 선택)으로 보내도록 정리함
- **완료 추가**: 고객 링크 흐름은 `/consultation`에서 스플래시 후 customer-link 전용 플로우로 즉시 라우팅하고, 매장(staff) 전용 디자이너 선택 화면은 `entry=customer-link`일 때 노출되지 않도록 분리함
- **완료 추가**: customer-link 루트 진입 시 시작 단계를 고객용 상담 페이지(`/consultation/customer`)로 고정해, `/consultation/step1`(현장용 기본조건 화면)으로 잘못 들어가던 문제를 수정함
- **완료 추가**: customer-link의 URL은 `/consultation`으로 유지하되, 미들웨어 rewrite로 고객용 페이지를 렌더링하도록 변경해 브라우저 주소가 `/consultation/customer`로 바뀌지 않게 정리함
- **완료 추가**: customer-link에서는 `/consultation` 루트 자체가 고객 정보 화면을 직접 렌더링하도록 조정하고, traits/summary에서 고객 정보 단계로 돌아갈 때도 `/consultation` 경로를 사용해 고객/현장 플로우를 명확히 분리함
- **완료 추가**: 상담 순서를 `고객 정보 → 기본 조건 → 시술 유형 → 고객 성향 → 최종 확인`으로 재정렬해 고객 정보를 Step 1로 당기고, staff 플로우에서 고객 정보 다음 단계가 `/consultation/step1`로 이어지도록 조정함
- **완료 추가**: customer-link도 고객 정보 Step 1 이후 바로 요약으로 건너뛰지 않고 `/consultation/step1 → /consultation/step2 → /consultation/traits → /consultation/summary` 순서로 이어지도록 수정해, `다음` 클릭 시 Step 5로 점프하던 오동작을 제거함
- **완료 추가**: customer-link 포트폴리오 선택은 같은 이미지를 다시 누르면 즉시 선택 해제되도록 토글 UX로 바꾸고, `BDX 포트폴리오 크게 보기` 전체화면 브라우저에서 더 큰 실사진을 감상하며 선택/취소할 수 있게 보강함. 이 브라우저는 링크의 `shopId` 기준으로 포트폴리오를 직접 불러와 로그인 세션과 무관하게 동작함
- **완료 추가**: customer-link URL에 `designerId`/`customerId`도 함께 실어 보내고, 저장 시 더 이상 직원 로그인 세션(`currentShopId`, `activeDesignerId`)을 전제로 하지 않도록 수정함. 외부 디바이스에서는 링크 컨텍스트 + `dbCompletePreconsultationBooking()`만으로 사전 상담을 저장하고, 성공 후에는 `/home` 대신 customer-link 전용 완료 화면으로 이동함

### 8.2 🟢 매장 태블릿 모드
- **완료**: 매장 태블릿 모드 구현됨
- **완료 추가**: 현장 상담 시작 시 선택한 디자이너를 연결된 예약에도 즉시 동기화해 `records` 탭 디자이너 캘린더에서 해당 예약이 선택한 디자이너 열에 유지되도록 정리함
- **완료 추가**: 홈 `새 예약 등록` 모달도 제출 시 `designerId`를 함께 저장하도록 보강해, 예약 생성 직후부터 `records` 디자이너 캘린더가 `미지정`이 아닌 선택한 담당 선생님 열에 바로 배치되게 수정함

---

## 9. 캔버스 → 옵션 기능으로 분리

> 핵심: 손가락별 디자인 캔버스는 **모든 샵이 필요한 건 아님**

### 9.1 🟢 캔버스 옵션화
- **완료**: 캔버스가 옵션 기능으로 분리됨

---

## 10. 시술 리포트 기념품화 — 외국인 관광객 추억 콘텐츠 ⏳ MVP 이후

> 핵심: 시술 후 **"저장하고 자랑하고 싶은" 디지털 카드** 자동 생성
> **MVP 이후 구현 예정** — 핵심 CRM 기능 완성 후 진행

### 10.1 ⬜ 비주얼 다이어리 카드 `MVP 이후`
- 매거진 레이아웃으로 [시술 전 + 시안 + 시술 후] 배치. 이미지 저장 가능

### 10.2 ⬜ QR코드 리포트 접근 `MVP 이후`
- 시술 완료 시 QR코드 생성 → 고객 스캔 시 리포트 접근

### 10.3 ⬜ K-뷰티 인증 마크 / 샵 낙관 `MVP 이후`
- "Certified by BDX" 로고 + 샵 커스텀 낙관

### 10.4 ⬜ 퍼스널 케어 팁 (다국어) `MVP 이후`
- 피부톤 맞춤 컬러 추천 + 관리 팁 (ko/en/zh/ja)

### 10.5 ⬜ SNS 공유 (인스타 스토리용) `MVP 이후`
- 인스타 스토리 공유 버튼 + 샵 계정/해시태그 자동 삽입

### 10.6 ⬜ 비포/애프터 숏폼 영상 `MVP 이후`
- 15초 Reels/TikTok용 영상 자동 생성

### 10.7 ⬜ 친구 추천 할인 코드 (Referral) `MVP 이후`
- 리포트 하단 할인 코드 삽입

---

## 11. 대시보드 — 매출 중심 → 성과·현장 중심

> 핵심: 원장님이 **"BDX 덕분에 돈 더 벌었네"**를 체감하는 지표

### 11.1 🟢 업셀링 리포트
- **완료**: 대시보드 상단에 `업셀링 리포트` 와이드 카드를 추가해 상담 기반 디자인/표현기법/파츠/컬러 추가 매출을 별도 집계해 표시함. 업셀링 적용 상담 수, 상담당 평균 추가 매출, 가장 크게 기여한 옵션까지 함께 노출함

### 11.2 🟢 글로벌 대시보드
- **완료**: `오늘의 외국인 예약` 수와 언어별 예약 건수/준비 완료 건수/준비율을 한 카드에서 확인할 수 있도록 구현함. 예약 언어와 사전 상담 완료 상태를 실데이터 기반으로 집계해 BDX의 다국어 운영 강점을 시각화함

### 11.3 🟢 재방문 골든타임 알림
- **완료**: 평균 주기 28일 기준 이번 주 리마인더 대상 고객을 자동 선별해 리스트로 표시하고, 고객별 맞춤 리마인더 문구를 `문구 복사` 버튼으로 즉시 복사할 수 있게 구현함
- **완료 추가**: 라이브 DB의 `customers` 테이블은 anon 권한에서 RLS로 보호되어 직접 seed insert가 막히므로, 실제 테스트 샵(`shop-1773300748626-x8a073` / 테스트상점) 에만 3명의 월간 재방문 고객 더미를 hydrate 시점에 병합해 `재방문 골든타임 알림` 카드를 바로 확인할 수 있게 함

### 11.4 🟢 디자이너 지표 고도화
- **완료**: `DesignerPerformance` 위젯에 디자이너별 매출, 예약 배정률, 상담 완료율, 완료 예약 건수를 추가하고, 상담 건수 차트와 함께 운영 지표용 바 차트를 확장함. 현재 데이터 구조상 `지정 예약률` 대신 확인 가능한 `예약 배정률`로 안전하게 반영함

### 11.5 🟢 VIP 고객 리스트 key 안정화
- **완료**: `CustomerAnalytics` 의 VIP 고객 TOP 5가 `name` 대신 고객 `id` 를 key로 사용하도록 집계 타입과 렌더링을 함께 수정해, 동명이인 고객이 있어도 React 중복 key 경고 없이 안정적으로 렌더링되게 함

### 11.6 🟢 대시보드 지표 재배치 및 용어 정리
- **완료**: 업셀링 리포트는 상단 핵심 카드로 유지하되, `업셀링 포인트`를 파츠/컬러 추가 카운트 기준으로 재정리하고 불필요한 보조 블록을 제거함
- **완료 추가**: 외국인 예약 현황과 재방문 골든타임 알림 카드를 하단으로 이동하고, KPI 카드는 4개만 남기도록 축소함. 중복되던 `선생님별 상담 건수`, `피크타임`, `인기 시술 Top 3`, `단골 방문율`을 제거하고 대시보드 내 `VIP` 표기도 `단골`로 통일함

---

## 12. 디자인 트렌드 분석

> 핵심: 샵 내 인기 디자인 랭킹으로 **포트폴리오 + 마케팅 강화**

### 12.1 ⬜ 인기 디자인 랭킹
- **지금**: 포트폴리오 실데이터 썸네일은 연결됐지만, `이번 달 인기 디자인 랭킹` 자체를 별도 기준으로 집계/정렬하는 로직은 없음
- **할 일**: 실제 상담/시술 데이터 기반으로 `이번 달 인기 디자인` 랭킹을 자동 산출하고, 포트폴리오 사진/가격/고객 반응 데이터와 연동

### 12.2 ⬜ 트렌드 알림
- **할 일**: 랭킹에 오른 시술의 고객에게 알림 → 동기부여/재방문 유도

---

## 13. 인증 — 구글 로그인 + 실제 인증 시스템

> 핵심: 현재 **인증이 전혀 없음** (로그인 버튼 누르면 검증 없이 통과). 실서비스를 위해 구글 OAuth 필수

### 13.1 🔵 Supabase Auth 이메일 로그인 연결
- **지금**: `src/app/(auth)/login/page.tsx`, `src/app/(auth)/signup/page.tsx`, `src/store/auth-store.ts`가 Supabase Auth `signUp` / `signInWithPassword` / `getSession` 기반으로 실제 샵 계정 로그인을 처리함. 회원가입 시 빈 `shops` row와 owner `designers` row를 만들고, 이후 세션은 `currentShopId` 기준으로 복원됨
- **완료 추가**: `로그인 없이 서비스 체험하기` CTA는 이제 `demo@bdx.kr` Supabase 데모 계정으로 바로 로그인하고, `shop-demo` 샵의 seeded 데이터(고객/예약/기록/포트폴리오)로 진입함
- **완료 추가**: 배포 환경에서 Supabase 공개 env가 빠졌을 때는 placeholder 네트워크 호출 대신 명시적인 설정 오류 메시지를 반환하도록 auth bootstrap을 보강함
- **완료 추가**: 설정 `앱` 탭에 남아 있던 로컬 전용 비밀번호 변경 카드를 제거해, 실제 Supabase Auth 비밀번호 재설정이 아직 없는 상태에서 오해를 주는 가짜 변경 UI가 노출되지 않게 정리함
- **완료 추가**: 설정 `앱` 탭에서 로그아웃과 같은 동작을 하던 중복 `계정 전환` 버튼을 제거해, 인증 액션을 하단 로그아웃 버튼 하나로 단순화함
- **완료 추가**: 좌측 사이드바 하단의 역할 배지를 제거해 샵 이름 영역을 더 단순한 매장 식별 UI로 정리함
- **완료 추가**: 설정 `선생님 관리` 섹션에서 디자이너 프로필 추가/수정/삭제를 지원하고, 고객·예약·상담·메모에 연결된 디자이너 삭제는 안전하게 차단함
- **완료 추가**: 디자이너 프로필 이미지를 Supabase Storage `designer-profile-images` 버킷에 저장하도록 전환해, 설정 업로드/삭제 결과가 앱 전체 아바타에 바로 반영되게 함
- **완료 추가**: 이메일 회원가입과 Google 회원가입 마무리 화면에 `[필수] 이용약관` / `[필수] 개인정보 수집 및 이용` 동의 블록을 추가하고, 두 항목 모두 체크해야 회원가입이 진행되도록 가드함
- **완료 추가**: `/terms`, `/privacy` 전문 페이지를 추가해 한국형 웹서비스 약관/개인정보 처리방침 구조(사업자 정보 placeholder, 수집 항목, 이용 목적, 보관 기간, 위탁/국외 이전, 정보주체 권리, 보호책임자 문의처 등)를 한 번에 확인할 수 있도록 연결함
- **완료 추가**: 미들웨어 공개 라우트에 `/terms`, `/privacy`를 포함해, 비로그인 상태의 회원가입 화면에서도 약관 전문 페이지가 로그인 리다이렉트 없이 바로 열리도록 보강함
- **완료 추가**: 이용약관 본문을 조문형(`제1조`, `제2조` ...) 한국 SaaS 문체로 재정리하고, BDX에 아직 없는 seat 과금/환불 조항은 제외한 채 계정, 데이터 책임, 이용 제한, 해지, 보존/파기, 손해배상 구조로 맞춤화함
- **완료 추가**: 개인정보 처리방침도 적용 범위, 처리 목적, 항목, 보유기간, 파기, 위탁, 국외 이전, 제3자 제공, 권리 행사, 쿠키, 비적용 범위, 보호책임자, 권익침해 구제, 변경 고지까지 조문형 한국 문체로 재구성하고, BDX에 없는 유료 구독/웨비나/광고 추적 조항은 제외함
- **완료 추가**: 회원가입 동의 UI에서 카드형/다크 바 처리 없이 밝은 배경 위에 바로 놓이는 인라인 약관 문장형 레이아웃으로 정리하고, `이용약관` / `개인정보 수집 및 이용` 링크만 은은하게 강조되도록 조정함
- **남은 일**: 이메일 인증 UX, 비밀번호 재설정, 에러 메시지 정교화, 서버 측 세션 검증 추가

### 13.2 🔵 구글 OAuth Provider 연동
- **지금**: `src/app/(auth)/login/page.tsx`, `src/app/(auth)/signup/page.tsx`, `src/store/auth-store.ts`에 Google OAuth 진입 버튼과 `signInWithOAuth` 흐름을 연결했고, `src/app/auth/callback/route.ts` + `src/middleware.ts`에서 Supabase SSR 기반 코드 교환/세션 복원을 처리함
- **완료 추가**: Google 로그인에서 아직 샵이 없는 계정은 `src/app/(auth)/signup/google/page.tsx`로 보내 이름/샵 이름을 입력받은 뒤 바로 샵 생성 + 온보딩으로 이어지도록 통합함
- **완료 추가**: 앱 공통 도메인 상수를 `https://beauty-decision.com`으로 도입하고, `metadataBase`/canonical/Open Graph URL 및 intro-demo 공개 링크를 모두 새 프로덕션 도메인 기준으로 통일함
- **완료 추가**: 루트 메타데이터의 Open Graph / Twitter 공유 이미지를 `public/bdx-og-image-final.png`로 교체해, 새 브랜드 비주얼이 링크 미리보기에도 바로 반영되도록 정리함
- **남은 일**: Supabase Dashboard의 Google provider에 Client ID/Secret 입력, Google Cloud Console에 `https://pzwmqorvrhdkckkdqemo.supabase.co/auth/v1/callback` 및 앱 callback URL 등록. 로그인 장애 발생 시 callback/provider 설정과 Google 계정 보안 상태(2차 인증/차단)까지 함께 점검하는 운영 체크리스트 추가

### 13.3 ⬜ 미들웨어 인증 가드
- **지금**: `src/middleware.ts`가 보안 헤더만 추가하고 **인증 여부를 전혀 검증하지 않음**. 로그인 안 해도 모든 페이지 접근 가능
- **할 일**: NextAuth 세션 기반으로 미들웨어에서 토큰 검증. 비로그인 시 `/login`으로 리다이렉트. 공개 라우트(login, splash, intro)만 예외 처리

### 13.4 🔵 인증 상태 관리 전환
- **지금**: `useAuthStore`가 Supabase 세션을 `initializeAuth()` / `onAuthStateChange()`로 복원하고, `owner_id = auth.user.id` 기준으로 현재 샵/원장 프로필을 찾아 `currentShopId`, `activeDesignerId`, 온보딩 완료 여부를 세션 컨텍스트로 유지함
- **완료 추가**: 샵 row가 아직 없거나 연결되지 않은 세션은 `fetchShopByOwnerId().maybeSingle()`로 조용히 null 처리해, auth bootstrap 시 불필요한 콘솔 에러가 뜨지 않게 정리함
- **남은 일**: 현재는 owner 샵 계정 중심 인증만 지원함. 디자이너 개별 계정, 서버 검증 미들웨어, 권한 세분화는 추가 필요

---

## 14. 데이터베이스 — mock 데이터 → 실제 DB

> 핵심: 현재 **DB가 전혀 없고 모든 데이터가 localStorage**. 브라우저 초기화하면 데이터 전부 날아감

### 14.1 🟢 DB 서비스 선정 및 연결
- **완료**: Supabase PostgreSQL (Auth + DB + Storage 통합) 연결 완료. 8개 테이블에 실제 데이터 운영 중

### 14.2 🟢 DB 스키마 설계 및 보완
- **완료**: 8개 테이블(`shops`, `designers`, `customers`, `customer_tags`, `small_talk_notes`, `consultation_records`, `booking_requests`, `portfolio_photos`) 운영 중
- **완료 추가**: 누락 컬럼 마이그레이션 — `booking_requests.pre_consultation_data/pre_consultation_completed_at`, `consultation_records.language`, `customers.preferred_language`, `shops.settings`(JSONB) 추가
- **완료 추가**: RLS 보안 강화 — `USING(true)` 일괄 삭제 → `get_my_shop_id()` 헬퍼 함수 기반 shop 격리 정책으로 교체. anon INSERT(QR 예약)도 별도 정책 허용

### 14.3 ⬜ API 라우트 구현
- **지금**: `src/app/api/` 라우트 없음. 모든 데이터 조작이 클라이언트 Zustand store에서 직접 Supabase SDK 호출
- **할 일**: Next.js App Router API 라우트(`app/api/`) 생성. 서버 측 검증 추가

### 14.4 🟢 Zustand store → DB 연동 전환
- **완료**: Supabase hydrate 대상인 `shop`, `customers`, `consultation_records`, `booking_requests`, `portfolio_photos`는 현재 로그인된 `currentShopId` 기준으로만 읽고 쓰도록 스토어/DB helper를 정리해 샵 간 데이터가 섞이지 않게 함
- **완료 추가**: Shop settings(가격/서비스/시간 설정)를 `shops.settings` JSONB로 DB 동기화. `setShopSettings()` 호출 시 fire-and-forget DB write, `syncShopSettingsFromShop()` 시 DB에서 복원

### 14.5 🟢 코드 정리
- **완료**: `src/data/mock-dashboard.ts` 데드코드 삭제 (어디서도 import 안 됨)
- **완료**: `KPICards.tsx` 하드코딩 드릴다운 → 실시간 store 데이터 기반 계산으로 교체
- **완료**: `database.ts` Supabase 타입 재생성 (새 컬럼/함수/관계 반영)

### 14.6 🟢 이미지 스토리지
- **완료**: 포트폴리오 사진을 Supabase Storage `portfolio-images` 버킷에 업로드하고, 메타데이터는 `portfolio_photos` 테이블에서 관리하도록 전환함. 앱은 공개 URL 기반으로 이미지를 렌더링하고, 포트폴리오 스토어를 DB hydrate 흐름에 포함시킴
- **완료 추가**: 기존 `bdx-portfolio` localStorage base64 사진은 첫 hydrate 시 Supabase로 순차 이관하고, 업로드/삭제/초기화 결과를 사용자 피드백으로 노출함

---

## 15. 기록 관리 — 상담 이력 탐색 UX

> 핵심: 기록 탭에서 **상태 의미와 상세 동선이 직관적**이어야 함

### 15.1 🟢 상담 상태 표시
- **완료**: 기록 탭 예약 화면 상단에 `상담 상태`를 추가해 `상담 필요` / `상담 완료` 상태를 색상 점과 함께 바로 이해할 수 있게 정리함

### 15.2 🟢 상담 기록 검색 QA
- **완료**: 기록 탭 검색 입력 아래에 `이름 / 전화번호 / 서비스명` 기준을 명시해 실제 검색 범위를 화면에서 바로 확인할 수 있게 보강함
- **완료 추가**: QA 체크리스트를 다음 기준으로 정리함 — `이름 부분 검색`, `전화번호 포함 검색`, `서비스명(원컬러/풀아트 등) 검색`, `고객 필터(customerId)와 병행 시 결과 유지`, `기간 필터/태그 필터와 동시 적용 시 결과 일관성`, `DB hydrate 후 새로고침 시 동일 결과 유지`

### 15.3 🟢 `기록 상세보기` 동선 정리
- **완료**: 기록 리스트의 미리보기 CTA를 `기록 미리보기`, 미리보기 모달의 상세 CTA를 `기록 상세보기`로 통일하고, 예약/상담 상세 제목도 `기록 상세` 기준으로 정리함
- **완료 추가**: 고객 이동 CTA는 `고객 상세로 이동`으로 분리해 기록 상세 흐름과 고객 상세 흐름이 명확히 구분되도록 정리함
- **완료 추가**: 모바일 기록 리스트 카드에서 썸네일/상담 배지/시간/고객명/담당자/가격/미리보기 버튼이 한 줄 폭을 과하게 경쟁하던 구조를 정리하고, CTA를 아래로 내리며 표현기법 태그는 모바일에서 2개 + `+N`으로 축약해 가독성을 높임
- **완료 추가**: 기록 메인 탭과 요약 카드 카피도 `예약 관리` / `이번 주 예약` / `오늘 남은 예약`에서 `상담 관리` / `이번 주 상담` / `오늘 남은 상담`으로 바꿔, 기록 화면이 상담 중심 운영 흐름으로 읽히도록 정리함
- **완료 추가**: 디자이너 데이 그리드의 `미지정` 열은 항상 첫 열에 유지되도록 조정해, 배정 여부와 무관하게 열 구조와 드래그 기준이 고정되도록 정리함
- **완료 추가**: 예약 상세 바텀시트에 `고객용 상담 링크` 버튼을 추가하고, 링크 생성 UI도 같은 바텀시트 안에서 전환되도록 정리해 모달 위에 또 모달이 뜨지 않게 함. customer-link URL은 이름/전화/메모/언어/bookingId와 함께 `shopId`/`shopName`까지 포함해 바로 복사해 보낼 수 있음

---

## 16. 설정/운영 UX — 원장님 운영 흐름 정리

> 핵심: 설정은 **운영 정보 저장과 데모/프로필 관리가 명확**해야 함

### 16.1 🟢 운영 정보 저장 안정화
- **완료**: 운영 정보 저장 버튼이 실제 DB 저장 결과를 기다리도록 바꾸고, 저장 중 상태와 성공/실패 피드백 배너를 명시적으로 노출함
- **완료 추가**: 샵 기본 정보(`이름/전화/주소`)와 설정 정보(`addressDetail`)를 각각 저장한 뒤 둘 중 하나라도 실패하면 편집 모드를 유지하고 에러를 보여주도록 보강함

### 16.2 🟢 데모 모드/프로필 이미지 UX 정리
- **완료**: 설정의 `사용 가이드 다시 보기`를 실제 동작에 맞는 `데모 가이드 다시 보기`로 바꾸고, 홈에서 기능 안내를 다시 보여주는 액션임을 보조 문구로 명확히 안내함
- **완료 추가**: 선생님 관리의 프로필 이미지 제어를 아이콘-only 버튼 대신 `프로필 수정`, `사진 업로드/변경`, `사진 삭제`, `프로필 삭제` 텍스트 액션으로 정리해 중복된 X 버튼 혼동을 제거함

### 16.3 🟢 서비스 가격표 확장
- **완료**: 설정 > 서비스의 `기본 가격표`에 `리페어 개당`, `연장 추가금` 편집 항목을 추가해, 상담 화면에 보이는 `연장/리페어` 가격을 서비스 탭에서 직접 수정할 수 있게 보강함
- **완료 추가**: 저장한 `리페어/연장` 값은 `shopSettings.surcharges`에 반영되고, 상담 `ExtensionSelector` 카드 가격 표시와 실제 가격 계산(`price-calculator`)도 같은 값을 사용하도록 연결함
- **완료 추가**: 서비스 탭 하단에 `시술 항목 관리` 토글 카드를 추가해 오프/그라데이션/프렌치/마그네틱/파츠/리페어/연장/오버레이 노출 여부를 설정에서 직접 켜고 끌 수 있게 함

---

## 17. 개발 도구 — Supabase MCP

> 핵심: 개발 워크스페이스에서 Supabase 프로젝트를 MCP로 안전하게 조회할 수 있어야 함

### 17.1 🟢 Supabase MCP 워크스페이스 연동
- **완료**: 루트 `.mcp.json`의 Supabase MCP 서버를 `read_only=true`로 고정해 기본 조회 전용으로 연결하고, 최초 브라우저 인증 및 `project_ref` 스코프 옵션을 개발 문서에 기록함

### 17.2 🟢 글로벌 CSS 폰트 import 정리
- **완료**: `next/font/local`로 이미 로드 중인 Pretendard를 `src/app/globals.css`에서 CDN `@import`로 중복 선언하던 부분을 제거해, dev 서버가 `@import rules must precede all rules` CSS 파싱 오류로 500을 내던 문제를 해결함

---

## 18. QA 검증 — 2026-03-12 브라우저 자동화 + 코드 분석

> 핵심: Playwright 브라우저 자동화로 실제 플로우를 E2E 테스트하고, 코드 분석으로 알림/상담 로직을 검증

### 18.1 🔴 (main) 페이지 i18n 언어 누수 버그
- **지금**: SideNav 하단 언어 드롭다운에서 언어를 변경하면 `(main)/*` 페이지 전체가 해당 외국어로 전환됨
- **원인**: `layout.tsx`의 `useLayoutEffect([setLocale])`이 최초 마운트 1회만 실행. 이후 SideNav `setLocale('ja')` 호출을 되돌릴 수 없음
- **할 일**: `useLayoutEffect` 의존성에 `locale` 추가하거나, (main) 컨텍스트에서 SideNav 언어 변경 UI 제거

### 18.2 🔴 크로스 기기 실시간 알림 미구현
- **지금**: 고객이 customer-link로 사전상담을 제출하면 Supabase DB에는 저장되지만, 사장님 브라우저에는 새로고침/재로그인 전까지 반영 안 됨
- **원인**: Supabase Realtime 구독이 없음. `hydrateFromDB()`는 앱 로드 시에만 호출
- **할 일**: `booking_requests` 테이블에 Supabase Realtime 구독 추가, 또는 polling 메커니즘 구현

### 18.3 ⬜ customer-link 라우트 접근 제어
- **지금**: customer-link 고객이 URL 직접 입력으로 `/consultation/step3`, `/consultation/canvas` 등 staff 전용 라우트에 접근 가능
- **할 일**: 각 라우트에서 `isCustomerLinkFlow` 확인 후 staff 전용 기능 차단 또는 리다이렉트

### 18.4 🟢 사전상담 알림 플로우 E2E 검증 완료
- **완료**: 로그인 → 상담 링크 생성 → customer-link 진입 → 스플래시 → 5단계 상담 → 제출 → 알림 확인 전체 흐름 Playwright로 검증
- **정상 확인**: 알림 벨 카운트, 배너 표시, 예약 상태 전환(현장상담→확정), 매출/상담 통계 반영

### 18.5 🟢 고객 상담 링크 플로우 E2E 검증 완료
- **완료**: URL 파라미터 프리필, 기존 고객 검색 숨김, 할인 UI 숨김, staff 버튼 숨김, save-complete 고객 전용 뷰 모두 정상

### 18.6 🟢 agent-browser QA 운영 규칙 정리
- **완료**: `docs/qa-agent-browser.md`를 추가하고, 프로덕션 QA에서 같은 세션을 병렬 재사용하지 않는 규칙, 세션 네이밍 규칙, 종료/정리 절차를 문서화함
- **완료 추가**: `package.json`에 `pnpm qa:agent:*` 실행 스크립트를 추가하고, `scripts/agent-browser.mjs`로 프로덕션 페이지별 안전한 세션 이름 생성과 `open` / `status` / `close` 명령을 공통화함

---

## 19. 모바일 + 태블릿 반응형 최적화 — 2026-03-12

> 핵심: iPad mini(768px)에서 SideNav가 콘텐츠를 압축하는 근본 문제 해결 + 스크린샷 확인된 버그 8건 + 코드 분석 기반 25건 수정

### 19.1 🟢 SideNav/BottomTabBar breakpoint `md:` → `lg:` (1024px)
- **완료**: AppShell, StatusBar, BottomTabBar, SideNav의 모든 `md:` breakpoint를 `lg:`로 변경. iPad mini(768px)에서 SideNav 대신 BottomTabBar 표시. globals.css에 `mb-safe`, `pb-safe`, `safe-area-inset` 유틸리티 추가, `overscroll-behavior-y: none` 적용, deprecated `WebkitOverflowScrolling` 제거

### 19.2 🟢 모달 태블릿 SideNav 가려짐 수정
- **완료**: Modal.tsx의 `md:` → `lg:` 변경, `lg:` 이상에서 SideNav 오프셋 반영 (`lg:left-[calc(100px+50%)]`), `max-h-[90vh]` → `max-h-[90dvh]` 키보드 대응, `pb-safe` 추가

### 19.3 🟢 설정 선생님 목록 레이아웃 수정
- **완료**: 카드 레이아웃을 `flex-row` → `flex-col`로 변경. 상단: 아바타+이름+뱃지, 하단: 액션 버튼 flex-wrap

### 19.4 🟢 대시보드 "문구 복사" 버튼 잘림 수정
- **완료**: 텍스트 div에 `min-w-0 flex-1`, 버튼에 `shrink-0` 추가

### 19.5 🟢 포트폴리오 업로드 2열 필드 겹침 수정
- **완료**: `sm:grid-cols-2` → `lg:grid-cols-2`

### 19.6 🟢 할인 모달 키보드 대응 + 프리셋 칩 UI
- **완료**: 적용/초기화 버튼 `sticky bottom-0`, 할인 비율 프리셋 [5%/10%/15%/20%/직접입력], 예약금 프리셋 [₩10,000/₩20,000/₩30,000/직접입력]

### 19.7 🟢 메시지 복사 버튼 피드백 UX
- **완료**: 클릭 시 "복사 완료!" 텍스트 + `bg-green-600` 색상 변경 (2초 후 복원)

### 19.8 🟢 Major 반응형 수정 11건
- **완료**: ConsultationLocaleButton safe-area, StatusBar 드롭다운 위치, ConsultationHeader truncate, ConsultationFooter 버튼 min-w, CustomerInfoForm 세로 스택, Calendar min-w 반응형, ColorPicker label overflow, PartsSelector truncate, customers/[id] nested scroll 제거, TodayStatsCard 가독성, BottomTabBar truncate
- **완료 추가**: `records` 모바일 예약 상세 바텀시트를 `max-h + flex column + 내부 scroll` 구조로 바꿔, 주의사항/참고사항 카드가 길어져도 하단 액션(`상담 시작`, `수정`, `닫기`, `삭제`)까지 끝까지 스크롤할 수 있게 수정함
- **완료 추가**: 같은 패턴으로 홈 `사전 상담 내용`, 기록 `상담 미리보기`, 대시보드 KPI 바텀시트도 내부 스크롤 구조로 통일해, 긴 메모/이미지/상세 리스트가 있어도 모바일에서 하단 내용이 잘리지 않게 정리함
- **완료 추가**: `records` 주간 캘린더는 Monday-start 주를 생성하면서도 셀 index로 요일 라벨을 붙이던 문제를 수정해, 날짜 숫자와 요일(`월~일`)이 KST 기준 실제 날짜와 일치하도록 정리함
- **완료 추가**: `records` 디자이너 데이 그리드의 `미지정` 열을 다시 항상 노출하도록 바꿔, 담당 선생님 배정 여부와 무관하게 고정된 열 구조로 일정 비교가 가능하게 정리함

### 19.9 🟢 Minor 터치/UX 수정 8건
- **완료**: 터치타겟 44px 보장 (4파일), OffSelector 배지 overflow, BodyPartSelector 패딩 최적화, ShapeSelector grid `lg:`, HandIllustration annotation truncate, records 태그 fade 힌트, customer 썸네일 통일, QuickActions label 가독성
- **완료 추가**: 모바일 `StatusBar`에서 매장명 옆 역할 배지를 제거해, 상단 헤더가 더 짧고 안정적으로 보이도록 정리함

---

## 20. BDX 2.0 MVP 보완 (2026-03-23)

> Phase 0~5 타입 확장 및 기능 보완. 상세: `docs/production/mvp-update-checklist.md`

### 20.1 🟢 운영 효율성 — 캘린더
- **완료**: 빈 슬롯 롱프레스 예약 폼, 드래그 앤 드롭 확인 모달(`DragConfirmModal`), 네이버 예약 채널 프리셋, 예약 폼 `initialValues` 지원
- **완료 추가 (2026-05-29, CF-6)**: 태블릿에서 스케줄 블록 드래그가 안 되던 문제 근본 해결. `SlotColumn`의 빈슬롯 터치 핸들러(`onTouchStart/Move/End`)가 블록 터치를 가로채 드래그를 막던 것을 제거(빈슬롯 롱프레스 예약 모달 폐지 — 상단 `예약 추가` 버튼으로 대체). 완료(`completed`) 예약도 시간대 변경 드래그 허용(취소만 고정). 블록은 `motion.div + touchAction:none`로 iOS 터치 드래그 동작. (`73ee61d`)

### 20.2 🟢 상담 프로세스 최적화
- **완료**: 신규/재방문 분기(`consultation.type`), 재방문 단축 플로우(`RETURN_VISIT_STEP_ORDER`), 특이사항 태그 아이콘 (`traitIcon`)

### 20.3 🟢 고객 관리 확장
- **완료**: `durationPreference` 시술 시간 선호, 회원권 관리(`MembershipCard`), 비주얼 히스토리(`TreatmentPhotoCarousel`), 빠른 매출 등록(`QuickSaleForm`), 특이사항 아이콘 뱃지

### 20.4 🟢 포트폴리오 자산화
- **완료**: 태그 최소 3개 검증, 공개/비공개 토글(`isPublic`)

### 20.5 🟢 매출/대시보드
- **완료**: 결제 방법 선택(`PaymentMethodSelector`), 회원권 차감 로직, 오늘 매출 KPI 카드(`computeTodayRevenue`), 전월 대비 성장 지표 카드, i18n 키 4개 언어 보완

---

## 21. FE/BE/API/UX 종합 QA (2026-03-25)

> 3개 전문 에이전트(FE코드, BE/API, 사용자 페르소나)로 전수 점검 후 발견된 22건 수정

### Critical (2건)
- **N-1** 🟢 일반 상담 레코드에 `finalizedAt` 미설정 → 대시보드 매출 0원 (`summary/page.tsx`)
- **N-2** 🟢 STEP_ORDER에 STEP3_OPTIONS/CANVAS 추가 (`consultation-store.ts`)

### High (8건)
- **N-3** 🟢 QuickSaleForm `customerName` URL param 미처리 (`QuickSaleForm.tsx`, `quick-sale/page.tsx`)
- **N-4** 🟢 `initialCustomerId` 있을 때 고객명 자동 채우기 없음 (`QuickSaleForm.tsx`)
- **N-5** 🟢 QuickSale `paymentMethod` 레코드에 미저장 (`records-store.ts`)
- **N-6** 🟢 `addRecord` 일반 경로 DB 실패 시 로컬 롤백 없음 (`records-store.ts`)
- **N-7** 🟢 `dbUpsertCustomerTags` delete 성공 + insert 실패 시 태그 전부 소실 (`db.ts`)
- **N-8** 🟢 사전 상담 완료 알림을 AppShell로 이동 — 모든 메인 페이지에서 노출 (`AppShell.tsx`)
- **N-9** 🟢 return_visit 플로우에서 "기본값 설정" 안내 배너 추가 (`customer/page.tsx`)
- **N-10** 🟢 홈 폴링이 마운트 즉시 1회 호출 안 함 (`home/page.tsx`)

### Medium (12건)
- **N-11** 🟢 시술 이력 중복 제거를 recordId 기반으로 변경 (`customers/[id]/page.tsx`)
- **N-12** 🟢 weekStats 날짜 계산에서 KST 적용 (`records/page.tsx`)
- **N-13** 🟢 `fetchBookingRequestById` shopId 없으면 경고 (`db.ts`)
- **N-14** 🟢 customer DB 조작 에러 시 구체적 로그 (`customer-store.ts`)
- **N-15** 🟢 QuickSale 미등록 고객 이름 매칭 + 경고 로그 추가 (`records-store.ts`)
- **N-16** 🟢 매출 등록 완료 후 상담기록 탭으로 이동 (`quick-sale/page.tsx`)
- **N-17** 🟢 bookingId 딥링크 전체 예약에서 검색 (`records/page.tsx`)
- **N-18** 🟢 홈/대시보드 매출 집계 로직 통일 (`home/page.tsx`)
- **N-19** 🟢 고객 목록에 "+ 고객 추가" 버튼 추가 (`customers/page.tsx`)
- **N-20** 🟢 설정 편집 중 이탈 시 `beforeunload` 경고 추가 (`settings/page.tsx`)
- **N-21** 🟢 canvas 파츠 위치 복원 시 저장된 좌표 사용 (`canvas/page.tsx`)
- **N-22** 🟢 treatment-sheet discountAmount 정상 확인 완료 (수정 불필요)

### Low (8건) — 별도 라운드
- N-23~N-30 ⬜ 미착수

---

## 22. 미리 정하기 (Pre-Consult) — 고객 사전 선택 퍼블릭 플로우

> 핵심: 고객이 사전 링크를 받아 **혼자 미리 디자인/옵션을 선택**하는 비로그인 플로우

### 22.1 🟢 타입 정의 (`src/types/pre-consultation.ts`)
- **완료**: PreConsultStep, DesignCategory, NailCurrentStatus 등 10종 union type + 4개 인터페이스 (PreConsultationData, PreConsultationType, PreConsultPriceEstimate, ShopPublicData)

### 22.2 🟢 BookingChannel 확장
- **완료**: `BookingChannel`에 `'pre_consult'` 추가, 관련 exhaustive Record 3곳 수정

### 22.3 🟢 DB 마이그레이션 + Storage
- **완료**: `pre_consultations` 테이블 SQL 작성 (3개 인덱스), `pre-consult-refs` 버킷 설정 문서화
- **참고**: Supabase MCP 토큰 만료로 수동 적용 필요 (`supabase/migrations/20260403_create_pre_consultations.sql`)

### 22.4 🟢 Zustand 스토어 (`src/store/pre-consult-store.ts`)
- **완료**: localStorage persist (key: `bdx-pre-consult`), shopData/portfolioPhotos 제외 partialize, 전체 상태+액션 구현

### 22.5 🟢 가격 계산 (`src/lib/pre-consult-price.ts`)
- **완료**: 카테고리 기본가 + 제거/연장/추가옵션 서차지 계산, min/max 가격 + 예상 시간

### 22.6 🟢 DB 함수 (`src/lib/db.ts` 확장)
- **완료**: fetchShopPublicData, fetchPublicPortfolioPhotos, dbCreate/Update/CompletePreConsultation, uploadPreConsultImage 6개 함수

### 22.7 🟢 i18n 4개 언어 (`src/lib/i18n.ts`)
- **완료**: `preConsult` 네임스페이스 ko/en/zh/ja 90키×4언어 추가

### 22.8 🟢 레이아웃 + STEP 0 시작
- **완료**: Server+Client layout, PreConsultProgressBar, 시작 페이지 (샵 이름 + 언어 선택 + 시작 버튼), not-found 페이지

### 22.9 🟢 STEP 1 디자인 선택
- **완료**: CategoryPicker (2×2 그리드), DesignGallery (포트폴리오 필터 + 안심 문구), PriceRangeHint (예상 가격/시간), design/page.tsx (순차 reveal)

### 22.10 🟢 STEP 2 스마트 상담
- **완료**: ReferenceUpload, NailStatusSelector(+분기), LengthSelector(+분기), ShapePickerSimple, VibeSelector, StyleSelector(단일+복수), AdditionalOptions, ConsultReview, FatigueMessage, consult/page.tsx (8개 섹션 순차 reveal + 피로 관리 문구)

### 22.11 🟢 STEP 3 확정 + 예약
- **완료**: 4개 섹션 (최종 요약, 확정 가격 breakdown, 방문 안내, 예약 폼), DB 저장 + 라우팅

### 22.12 🟢 STEP 4 완료
- **완료**: 체크 애니메이션, 자동 저장 안내, 5초 후 스토어 리셋

### 22.13 🟢 손님 자유 입력 "추가 요청사항" 섹션 (2026-05-13)
- **완료**: 사전상담 `consult` 페이지에 8번째 섹션 `additionalRequest`(addons↔review 사이)를 추가하여 손님이 정형 옵션에 없는 요구사항(예: 알러지, 특정 손가락 포인트 제외, 길이 조절 등)을 자유 텍스트로 입력 가능. 500자 제한·카운터·선택 입력. `PreConsultationData.additionalRequest`(JSONB) 신규 필드로 `booking_requests.pre_consultation_data`에 저장됨(DB 마이그레이션 없음). `ConsultReview`에 요약 줄 노출, 사장님 `records/preconsult/[bookingId]`에 "💬 손님 요청사항" 카드로 표시. i18n 4개 언어 4키 추가.

---

## 23. 시술 후 결제 UX (STEP 6~11)

> treatment-sheet → /payment 결제 → 고객등록 → 사진저장 → 포트폴리오 연결

### 23.1 🟢 `/payment` 페이지 전면 재구현
- **완료**: 기존 redirect → 5섹션 순차 스크롤 페이지 전환. Section 1(시술 정리), Section 2(결제수단), Section 3(고객등록 조건부), Section 4(사진 저장), Section 5(완료)

### 23.2 🟢 `PriceBreakdown` 빌드 유틸
- **완료**: `src/lib/price-utils.ts` — `buildBreakdownFromRecord()` 함수. record.pricingAdjustments → PriceBreakdown 변환

### 23.3 🟢 고객 매칭 + 통계 갱신
- **완료**: 전화→이름→신규 생성 3단계 매칭 패턴 (records-store 패턴 재사용). visitCount/totalSpend/averageSpend/treatmentHistory 자동 갱신

### 23.4 🟢 사진 저장 + 포트폴리오 자동 연결
- **완료**: 카메라/갤러리 입력, FileReader data URL, portfolioStore.addPhoto(kind='treatment', isPublic=false), record.imageUrls + customer.treatmentHistory.imageUrls 업데이트

### 23.5 🟢 treatment-sheet Quick Add Chips (STEP 6)
- **완료**: extras 섹션 상단 가로 스크롤 프리셋 버튼 5종 (파츠 +2k, 글리터 +3k, 포인트 아트 +5k, 연장 추가 +10k, 기타). isPriceFinalized 시 숨김

### 23.6 🟢 회원권 결제 연동
- **완료**: PaymentMethodSelector membership 잔여 횟수 표시, useMembershipSession 호출, dbInsertMembershipTransaction 연동

---

## 24. 전체 심층 QA (2026-04-15) — 보안 + 데이터 정합성 + i18n

> 핵심: UX 개선 33건 커밋 후, Supabase 연동·보안·i18n·비즈니스 로직 전수 검증

### CRITICAL (4건)
- **QA-C1** 🟢 RLS `demo_open_*` 40개 정책 삭제 → shop-demo 한정 10개 재생성 (Supabase SQL)
- **QA-C2** 🟢 Ghost session localStorage 복구 로직 제거 (`auth-store.ts`)
- **QA-C3** 🟢 데모 쿠키 SameSite=Strict;Secure 추가 (`auth-store.ts`)
- **QA-C4** 🟢 djb2 → PBKDF2 SHA-256 100K iterations + 레거시 자동 마이그레이션 (`auth-store.ts`, `ProfileSwitcher.tsx`, `settings/page.tsx`)

### HIGH (8건)
- **QA-H1** 🟢 `booking_requests.deposit` DB 컬럼 추가 + db.ts 매핑 (Supabase SQL + `db.ts`)
- **QA-H2** 🟢 고객 통계 이중 카운팅 → `recordTreatmentCompletion` 통합 (`customer-store.ts` + 5곳 교체)
- **QA-H3** 🟢 treatmentHistory 3곳 누락 수정 (`records/page.tsx`, `records/[id]/page.tsx`, `payment/page.tsx`)
- **QA-H4** 🟢 fieldMode 63키 × 3언어 번역 (`i18n.ts`)
- **QA-H5** 🟢 treatment-sheet 72키 × 4언어 전체 i18n (`treatment-sheet/page.tsx`, `i18n.ts`)
- **QA-H6** 🟢 consultation/error.tsx i18n + Dual Language (`error.tsx`, `i18n.ts`)
- **QA-H7** 🟢 fetchBookingRequestById shopId 없으면 null 반환 (`db.ts`)
- **QA-H8** 🟢 미들웨어 env 미설정 시 보호경로 → /login 리다이렉트 (`middleware.ts`)

### MED (12건)
- **QA-M1** 🟢 BookingStage cancelled 상태 추가 + UI 반영 (`booking-stage.ts`, `TodayReservationCard.tsx`, `records/page.tsx`)
- **QA-M2** 🟢 settlement booking별 deposit 우선 사용 (`settlement/page.tsx`)
- **QA-M3** 🟢 settlement vs consultation 금액 → 별도 워크플로우로 설계 의도 확인 (버그 아님)
- **QA-M4** 🟢 isExternalCustomerLinkFlow 분기 rollback 추가 (`summary/page.tsx`)
- **QA-M5** 🟢 save 더블탭 race condition → useRef 즉시 차단 (`summary/page.tsx`)
- **QA-M6** 🟢 hydration shopId 스냅샷 stale 체크 (`SupabaseProvider.tsx`)
- **QA-M7** 🟢 dbUpsertCustomerTags shopId 필터 + insert shop_id 포함 (`db.ts`)
- **QA-M9** 🟢 pre-consult 폴백 shop-demo만 허용 (`pre-consult/layout.tsx`)
- **QA-M10** 🟢 CSP + HSTS 보안 헤더 추가 (`middleware.ts`)
- **QA-M12** 🟢 consultation '(선택)' i18n 적용 (`consultation/page.tsx`, `i18n.ts`)

### LOW (6건)
- **QA-L1** 🟢 dbUpdateCustomerField 필드 화이트리스트 (`db.ts`)
- **QA-L2** 🟢 PIN 해시 PBKDF2 교체로 해소
- **QA-L4** 🟢 consultation/loading.tsx 로딩 스피너로 대체 (`loading.tsx`)
- **QA-L5** 🟢 customer/page.tsx 재방문 배너 i18n (`customer/page.tsx`, `i18n.ts`)
- **QA-L7** 🟢 미사용 i18n 키 — 향후 사용 가능으로 유지

### DB 변경사항
- `booking_requests.deposit` integer 컬럼 추가 (default 0)
- `customer_tags.shop_id` text 컬럼 추가 + 백필
- RLS: `demo_open_*` 40개 삭제 → `demo_*_write` 10개 (shop-demo 한정)

---

## 진행 요약

| 영역 | 항목수 | 🟢 | 🟡 | 🔵 | ⬜ |
|------|--------|-----|-----|-----|-----|
| 홈/스케줄 | 7 | 7 | 0 | 0 | 0 |
 | 포트폴리오 | 5 | 5 | 0 | 0 | 0 |
 | 고객 카드 | 7 | 7 | 0 | 0 | 0 |
 | 시술 전 알림 | 2 | 2 | 0 | 0 | 0 |
 | 상담 UX | 4 | 4 | 0 | 0 | 0 |
 | 가격 입력 | 2 | 2 | 0 | 0 | 0 |
 | 상담 입력 방식 | 2 | 2 | 0 | 0 | 0 |
 | 캔버스 옵션화 | 1 | 1 | 0 | 0 | 0 |
 | 리포트 기념품화 | 7 | 0 | 0 | 0 | 7 (MVP 이후) |
 | 대시보드 | 6 | 6 | 0 | 0 | 0 |
 | 트렌드 분석 | 2 | 0 | 0 | 0 | 2 |
 | **인증** | **4** | **0** | **0** | **2** | **2** |
 | **데이터베이스** | **6** | **5** | **0** | **0** | **1** |
 | 기록 관리 | 3 | 3 | 0 | 0 | 0 |
| 설정/운영 UX | 2 | 2 | 0 | 0 | 0 |
| 개발 도구 | 2 | 2 | 0 | 0 | 0 |
| **QA 검증** | **6** | **3** | **0** | **0** | **1** |
| **반응형 최적화** | **11** | **11** | **0** | **0** | **0** |
| **BDX 2.0 MVP** | **20** | **20** | **0** | **0** | **0** |
| **종합 QA (N-1~30)** | **30** | **22** | **0** | **0** | **8** |
| **미리 정하기** | **12** | **12** | **0** | **0** | **0** |
| **시술 후 결제 UX** | **6** | **6** | **0** | **0** | **0** |
| **전체 심층 QA** | **32** | **30** | **0** | **0** | **0** |
| **0423 카톡 피드백** | **3** | **3** | **0** | **0** | **0** |
| **통계 정합성(0529)** | **5** | **5** | **0** | **0** | **0** |
| **E2E 검증 수정(0529)** | **2** | **2** | **0** | **0** | **0** |
| **상담 플로우 버그수정(0530)** | **10** | **10** | **0** | **0** | **0** |
| **사용자관점 QA(0530)** | **49** | **47** | **0** | **1** | **1** |
| **스케줄 일정 삭제(0601)** | **1** | **1** | **0** | **0** | **0** |
| **파츠 동기화·add-on 정리(0602)** | **1** | **1** | **0** | **0** | **0** |
| **고객 삭제(0602)** | **1** | **1** | **0** | **0** | **0** |
| **기본 파츠 fallback(0602)** | **1** | **1** | **0** | **0** | **0** |
| **파일럿 피드백(0604)** | **6** | **6** | **0** | **0** | **0** |
| **합계** | **258** | **229** | **0** | **3** | **22** |

---

## 20. 0423 카카오톡 피드백 반영 (지승호 대표)

> 출처: 2026-04-22 카톡 대화 · `client-fix/0423/0423-todo.md`

### 20.1 🟢 회원권 잔액 **금액 기반** 재설계
- **지금/완료**: `Membership` 타입에 `usedAmount`, `remainingAmount` 추가. `MembershipTransaction.amountDelta` 추가. `src/lib/membership.ts` 헬퍼(getRemainingAmount, calcMembershipDeduct) 신규. `useMembershipSession(customerId, recordId, amount)` 시그니처 확장 — 시술 금액을 넘기면 그만큼 차감, 생략 시 1회 단가 추정(하위 호환). `MembershipCard`가 "남은 금액 ₩X / 사용 ₩Y" 중심으로 전환, 횟수는 보조. 현장모드 정산에서 회원권 잔액 > 시술금이면 전액 차감, 잔액 < 시술금이면 차액은 현금/카드 선택 결제. 고객 상세·고객 목록·수동 차감 모달도 금액 기반 표시로 통일. `20260423_add_membership_amount.sql` 마이그레이션으로 `membership_transactions.amount_delta` 컬럼 추가.

### 20.2 🟢 재방문 알림 문구 **샵주인 커스터마이징**
- **지금/완료**: `ShopExtendedSettings.revisitMessageTemplate` 필드 추가. 설정 탭 > **고객 알림 > 재방문 알림 기본 문구** 섹션(`RevisitMessageSection`) 신설 — 기본 문구틀을 편집하고 `{customerName}` `{shopName}` 변수 삽입 버튼, 실시간 미리보기, 저장/기본값 되돌리기 제공. `RevisitReminderCard`가 저장된 템플릿을 `renderRevisitMessage`로 치환해 복사/SMS에 사용. 템플릿 미저장 시 기본값 fallback 유지.

### 20.3 🟢 공유카드 글씨 배열 다듬기
- **지금/완료**: `ShareCardImageTemplate`의 상단 해시태그 pill, 영문 타이틀(88→84px + letter-spacing 완화), 한글 서브(30px + 간격 14→10), 상담 메시지(22px + 간격 22→24), 샵 이름 블록(36→32px), QR 박스(120→108, 폰트 20→18, padding 재조정), FeedbackRow(padding 22/32 + minHeight 72)까지 타이포그래피 리듬을 재정돈. 디자인 톤은 유지한 채 "글씨 배열만" 정리.

## 21. 사용자관점 심층 QA (0530) — Chrome 실측 + 18에이전트 코드추적

> 상세: `docs/qa-deep-20260530-userqa.md`. 사전상담 링크 생성→제출→사장님 확인→시술→매출→대시보드→스케줄 전 구간을 Chrome(모바일 ~500px)으로 직접 걸어보며 조사. `tsc/lint/build` 통과, BUG-A·SL-04 라이브 재검증.

### 21.1 🟢 수정 완료 (24건)
- **사전상담 고객 플로우(6)**: DesignGallery 중첩버튼 hydration 에러 제거(div role=button) · 요약 raw 키 `consultation.designLabel` 수정 · SlotPicker 요일 다국어화(en/zh/ja) · ConsultReview 8라벨 i18n · DesignGallery/ReferenceUpload 하드코딩 i18n
- **파츠·가격(4)**: solidPoint/fullArt 설정가 견적 반영(SSOT 통일) · 랩핑 추가금 calculator 연결 · 견적=계산대 가격 일치(랩핑 폴백 통일) · app-store 기본값 정합
- **대시보드(6)**: 골든타임 최근시술 라벨 · 인기시술 카테고리/표현 분리 · 업셀링 finalized 필터 · 상담건수 KPI/차트/바텀시트 SSOT · 오늘매출 변화율 레이블 · 취소예약 캘린더 제외
- **스케줄(4)**: DragConfirmModal `__unassigned__` 정규화 · moveValidation 정규화 · 취소예약 색상 · 빈슬롯 모바일 롱프레스 touch 핸들러
- **포트폴리오(4)**: MenuCard 인라인편집 DB 영속화 · is_staff_pick/is_popular 삽입 · isPublic 명시 boolean · magnet 무드 매핑

### 21.2 🟢 후속 24건 수정 완료 (2차, SL-01 제외) — 상세 `docs/qa-deep-20260530-userqa.md` §2차 수정
- **DB 마이그레이션** `20260530_userqa_fixes`: portfolio_photos.parts_memo · uq_shoplink_slot 부분 UNIQUE 인덱스
- **매출/정산(8)**: FM-1 add-on 이중청구 차단(라이브검증) · SALES-1 즉시매출 회원권 차감 · SALES-2 예약금 totalSpend · SALES-3 wrap-up 고객변경 통계 · SALES-4 records/[id] breakdown · SALES-5 pricing_adjustments · SALES-6 금액권 차감 · 🔵 **FM-2 부분**(offType 수정, nailShape·partsSelections 미해결)
- **가격(4)**: PRICE-3 손님 견적=계산대 105,000 일치(랩핑 폴백 정정, 라이브검증) · customPartId · calculatePrice customParts · 환율 근사표시
- **사전상담/슬롯(5)**: SL-02 linkId 오염 · SL-03 샵링크 23505+인덱스 · SL-07 duration 겹침 · #20 미정 표시가드 · #13 complete 탈출버튼
- **포트폴리오(3)**: partsMemo 영속화 · PortfolioBrowser 공개사진만 · 카테고리 삭제 orphan
- **records 표시(2)**: SALES-4 · pre_consult_done '사전상담 보기'
- **반응형/i18n(4)**: 로케일버튼 분리 · 폰트/터치(12/44/10px) · html lang 동적 · (main) restoreLocale

### 21.3 ⬜ 여전히 미해결
- **SL-01** 사전상담 링크 설정 UI 부재 — 제품 결정 보류(사용자 지시)
- **FM-2 잔여** field-mode 매출레코드 nailShape·partsSelections 손실(데이터모델 작업 필요, final_price는 정확)
- is_quick_sale=true 플래그(설계 확인) · 환율 실시간 · customPart 모달 복원(부분)

## 22. 🟢 클라이언트 실측 버그 3건 (2026-05-30 3차) — 프로덕션 라이브

### 22.1 🔴→🟢 P0 사전상담/예약 제출 RLS 차단 (전 실제 샵)
- **증상**: 손님 사전상담 제출 시 `new row violates row-level security policy for table "pre_consultations"`
- **원인**: `pre_consultations`·`booking_requests` 의 `*_verified_insert` 정책 with_check 가 `EXISTS(SELECT 1 FROM shops WHERE id=shop_id)` 사용 → 이 서브쿼리가 **anon 의 shops RLS 하에서 평가**되는데 anon 은 `shop-demo` 외 실제 샵을 SELECT 못 함 → EXISTS=false → INSERT 거부. **shop-demo 만 동작, 모든 실제 샵의 고객 제출(경로 A/A-2/C) 전면 차단**
- **수정**: 마이그레이션 `20260530_fix_anon_submit_rls` — `shop_exists(text)` SECURITY DEFINER 함수(shops RLS 우회, boolean만 반환)로 with_check 교체. anon 역할 INSERT 실측 검증(pc/bk 모두 통과) 후 테스트행 롤백
- **P0-2 후속**: 로그인 사장님이 본인 고객화면 테스트 시 요청 역할이 `authenticated` → pre_consultations 에 authenticated INSERT 정책 없어 403 지속. 마이그레이션 `20260530_fix_submit_rls_authenticated` 로 두 정책을 `anon + authenticated` 로 확장. authenticated 역할 INSERT 실측 검증 완료. (실제 손님 anon 은 1차로 이미 해결, 사장님 테스트가 2차로 해결됨)

### 22.2 🟢 P1 시술 종류 SSOT 3중 분리 (포트폴리오 메뉴판 ↔ 설정 ↔ 사전상담)
- **증상**: 포트폴리오 메뉴엔 '흠흠' 보이는데 설정·사전상담엔 없음 (DB customCategories 엔 '테스트'만 존재)
- **원인**: `portfolio-store.menuCategories`(localStorage) 별도 리스트 유지 + `addCategory` 의 DB 동기화가 `void`(fire-and-forget)라 실패해도 무시 → localStorage 엔 흠흠, DB엔 없음(divergence)
- **수정**: `menuCategories` 로컬 상태 제거 → 포트폴리오 카테고리를 `shopSettings`(categoryLabels + customCategories)에서 파생(`deriveMenuCategories`). `addCategory/renameCategory/removeCategory` 를 `setShopSettings` await + 결과 반환(낙관적 업데이트·실패 롤백은 setShopSettings 처리)으로 변경 → 설정·사전상담과 항상 동일 집합

### 22.3 🟢 P1 "메뉴에 추가" 사진 첨부 안 됨
- **원인**: 픽커가 `kind==='treatment'` 사진만 노출 → 온보딩 업로드(`reference`) 사진을 메뉴에 못 넣음. 업로드 실패도 `void` 로 삼켜 무반응
- **수정**: 픽커 후보를 `kind` 무관 전체 비-featured 사진으로 확대. 업로드/카테고리 추가·변경 실패를 토스트로 노출

### 22.5 🟢 P1 샵 고정 링크 슬롯 미차단 (예약한 시간이 다음 손님에게 안 막힘)
- **증상**: 샵 고정 사전상담 링크에서 시간 선택 후 제출했는데, 그 슬롯이 슬롯 픽커에서 계속 예약 가능으로 표시됨
- **원인**: 샵 고정 링크 welcome 의 `canStart` 가 `linkIdParam` 일 때만 슬롯을 요구 → 슬롯 미선택でも 진행 가능 → confirm 에서 경로 C(`dbCompletePreConsultation`)로 빠져 `reservation_time='미정'` booking 생성. '미정' 은 어떤 슬롯과도 매칭 안 돼 `computeAvailableDates` 의 bookedSet 에서 영원히 제외됨. (실측: `bk-e4afe065` reservation_time='미정')
- **수정**: welcome `canStart` 를 `linkData && 가용슬롯>0` 이면 슬롯 선택 필수로 변경(`computeAvailableDates` 로 가용 여부 판정). 정상 흐름이 경로 A-2(`dbCreateBookingFromShopLink`, 실제 시간)로 가 booking 에 실제 시간 기록 → 슬롯 픽커가 line-through·카운트 감소로 차단. 버튼 텍스트도 canStart 와 일치. 슬롯 없는 샵(만석/휴무)은 종전대로 시간 없이 진행 허용
- 참고: RPC `get_shop_pre_consult_data` 는 booked slots(pending/confirmed)를 정상 반환하고 SlotPicker/`computeAvailableDates` 차단 로직도 정상 → 데이터(=실제 시간)만 들어오면 동작

### 22.6 🟢 P2 시술 종류 라벨 불일치 (설정 ↔ 포트폴리오 ↔ 사전상담)
- **증상**: 설정 시술종류는 "심플 / 마그네틱", 포트폴리오·사전상담은 "심플 / 원컬러 / 자석 / 마그넷" — builtin 라벨 텍스트가 화면마다 다름
- **원인**: 설정 page.tsx 가 인라인 라벨(`cat==='simple'?'심플':...`) 사용 → labels.ts `CATEGORY_LABELS`(canonical) 미사용
- **수정**: 설정을 `resolveMenuCategoryLabel(cat, categoryLabels)` 로 교체(라벨 칸 폭 w-16→w-28). 포트폴리오 `deriveMenuCategories` 도 동일 헬퍼 사용으로 통일 → 3곳이 labels.ts 단일 소스 공유(categoryLabels override 포함). tsc/build 통과
- **연동 정리(설계 확인)**: 카테고리 **집합**은 shopSettings(builtin 4 + customCategories) 단일 소스로 3곳 동기화. **사전상담 가격**은 해당 카테고리에 대표(featured) 포트폴리오 사진이 있으면 그 사진 가격을 "N원~"으로, 없으면 설정 기본가를 표시(getPriceHint) — 의도된 동작. (린업-테스트 심플 "1원~"은 ₩1 웨딩 사진, 설정 ₩500,000 은 별개 기본가/오타값)

### 22.7 🟢 설정 가격 연동 전수 감사 + 레거시 제거 (가격 모델 일원화)
- **연동 버그 수정**: 자샵/타샵 오프가 설정 저장 시 `baseOffSameShop/baseOffOtherShop`(상담)만 갱신되고 `surcharges.selfRemoval/otherRemoval`(사전상담·현장모드)는 미갱신 → 오프 가격이 사전상담/현장에 반영 안 됨. `handleSavePrices` 에서 두 표현 동기화
- **랩핑 추가금**: 수정 모드엔 있으나 읽기 화면 누락 → 읽기 행 추가
- **시술 종류 정렬**: custom 행 라벨 w-16→w-28 (builtin과 통일)
- **레거시 완전 제거**(사용자 결정): `/consultation` 화면은 진입점 없는 죽은 코드(홈·예약 시작 모두 /field-mode). `src/app/consultation/`·`src/components/consultation/`·`src/components/canvas/`·`use-consultation-guard.ts` 삭제(42파일), middleware PUBLIC_PREFIXES 정리. 설정에서 가산모델 전용 죽은 항목 5개(핸드/페디 기본·리페어·풀아트추가·이달의아트추가) 제거
- **가격 모델 현황**: 실제 운영 플로우(사전상담·현장모드·포트폴리오)는 **카테고리 모델**(`calculatePreConsultPrice` + categoryPricing 시술종류) 단일 사용. `calculatePrice`(가산모델)·`consultation-store`·`time-calculator` 는 삭제하지 않고 **records 상세·대시보드·analytics·홈·결제·캘린더의 내부 인프라로 유지**
- **검증**: clean tsc PASS / lint 0 error / build 성공

### 22.4 🟡 미확인
- **P2** 온보딩 완료 "저장 중 오류" — 인증 owner 저장 경로(updateShop/setShopSettings)라 P0와 별개. 브라우저 콘솔 로그로 재현 필요(commitDB가 uploaded/errors 카운트 로깅 중)
- **검증**: tsc/lint/build 통과. P0 DB 실측 검증 완료

## 23. 🟢 사전상담 통합 + 상태4단계 + 네이밍 + 포트폴리오 + 가격정합 (2026-05-30 대규모)
멀티에이전트(9 조사 + 6 구현) + Chrome 실측 QA. 커밋 95f2c03·6b018c4.
### 23.1 🟢 사전상담 내용 전 원장화면 통합 (최우선)
- 공용 `PreConsultDetailView`(이미지·디자인선택·네일상태·스타일키워드·추가옵션·요청사항, 가격/시간 계산 없음) 신설
- 임베드: records/preconsult(예상시간 제거)·records/[id](시술상세)·예약상세 모달(인라인)·**field-mode/treatment 접이식 섹션(실측 확인)**·customers/[id]
### 23.2 🟢 상태 4단계 (링크 미발송/응답 대기/응답 완료/시술 완료)
- booking-stage DisplayStage + DISPLAY_STAGE_LABELS/DOT. records 범례·타임그리드 뱃지·홈 카드 통일(실측). '신규' 뱃지·'미리 정하기'(채널라벨 '사전상담'으로) 정리
### 23.3 🟢 네이밍/시간/포트폴리오/가격
- 상담 시작→시술 시작(홈/기록/포폴/i18n), 상담 상세→시술 상세(실측)
- '미정'·비HH:MM → '시간 미정' 칩(NaN 제거, 실측). serviceLabel raw→한국어 시술종류
- 포트폴리오 저장 400(버킷 2MB 초과): 업로드 전 resizeTreatmentPhoto(1600px/~400KB) 강제 + db 가드
- wrap-up STEP9~11 삭제 + 고객 자동매핑(useEffect)
- 가격정합: records/[id] quick-sale 가산모델 재계산 → 저장 pricing_adjustments 사용(합계 92k vs 최종 47k → 시술금액 57k − 차감 10k = 47k, 실측). handleFinalize deposit 누락 수정. preconsult→field-mode wrappingPreference carry
- customers/[id]: 결제 합계·시술이력 드릴다운·예약이력(전화 매칭, 실측)
### 23.4 🟡 잔여(비파괴 QA 한계 — 사용자 인터랙티브 확인 권장)
- wrap-up 고객 자동매핑·포트폴리오 사진 업로드·예약상세 모달 인라인: 코드+빌드 검증 완료, 본인 Chrome에서 클릭 확인 권장 (MCP CDP 클릭 불안정으로 미실측)
- records/[id] '시술 리포트'는 저장된 consultation 스냅샷(핸드/단색+포인트)이라 카테고리(프렌치)와 표기 차이 — 가격 아닌 표시 스냅샷

## 24. 🟢 스케줄표에서 일정 바로 삭제 (2026-06-01 클라이언트 피드백 — 지승호 대표)
> 출처: 지승호 대표 카톡 — "스케줄표에 일정 취소로 삭제하고 싶으면? / 시술 완료 후엔 시술 상세 '이 기록 삭제'가 있는데 스케줄표에서 바로 삭제할 수 있을까요? / 잘못 입력했을 때 삭제하고 다시 할 경우 대비"
- **이전 상태**: 타임그리드 예약 시트는 `just_registered`·`cancelled` 단계에서만 삭제 버튼 노출. `link_sent`·`pre_consult_done`·`in_treatment`·`completed`는 삭제 불가 → 완료 건은 시술 상세로 들어가야만 삭제. 월간뷰 `DayReservationList`엔 삭제 기능 전무.
- **수정**:
  - 타임그리드 시트(`records/page.tsx`): 단계별 중복 삭제 버튼을 **단일 통합 삭제 버튼**으로 정리 → 모든 단계에서 노출. `executeDelete`가 연결된 시술 기록(`matchedRecord`)이 있으면 `removeRecord`(고객 방문/매출/이력 롤백)도 함께 호출 후 `removeReservation` → orphan 기록 방지. confirm 모달 문구를 완료 건(매출·통계 제외 경고)/일반 예약 조건부로.
  - 월간뷰 리스트(`DayReservationList.tsx`): 카드별 휴지통 아이콘(44px 터치타겟) + confirm 모달 추가. 동일하게 연결 기록 함께 삭제.
- **검증**: tsc EXIT 0 / lint 0 error(기존 img·deps 경고만) / build EXIT 0(37라우트). 인증 페이지라 무로그인 실측 불가 → 배포 후 확인.

## 25. 🟢 디자인 고르기 스톤/글리터 고정 add-on 제거 + 파츠 설정 동기화 (2026-06-02 클라이언트 피드백 — 지승호 대표)
> 출처: 지승호 대표 iPad 실측 — "사전상담·현장 디자인 고르기에서 스톤/글리터 부분 삭제 / 샵 제공 파츠가 설정과 동기화돼 고객에게 보여야 함 / 원장도 고객이 선택한 파츠 종류·수량 확인 가능해야 함"
- **이전 상태**: 사전상담(`AdditionalOptions`)·현장모드(`QuickOptionsPanel`) 둘 다 "더 완성도 있는 디자인…" 스톤(+5,000)/글리터(+3,000) 고정 add-on 노출. 글리터는 커스텀 파츠("글리터")와 중복. 파츠 동기화·원장 표시는 이미 구현돼 있었음.
- **수정**:
  - `AdditionalOptions.tsx`: 스톤/글리터 고정 add-on + 제목 제거, 단계 본문을 설정 동기화 커스텀 파츠 픽커로 승격. 파츠 0개 빈 상태 폴백(`partsListEmpty` 4언어 신규).
  - `QuickOptionsPanel.tsx`: Section 4(스톤/글리터 칩)·`addOnOptions`·미사용 `AddOnChip` 제거. 커스텀 파츠 섹션 유지. 랩핑은 그대로.
  - 요청 2(파츠 설정 동기화: `shopData.customParts`/`shopSettings.customParts`)·3(원장 확인: `PreConsultDetailView`의 `name ×count +₩total`)은 이미 충족 — 검증으로 확인.
- **검증**: tsc EXIT 0 / lint 0 error / build EXIT 0(37라우트). 상세 `docs/qa-deep-20260601.md` §6차.

## 26. 🟢 고객 등록 후 삭제 기능 (2026-06-02 클라이언트 피드백 — 지승호 대표)
> 출처: 지승호 대표 카톡(파일럿샵 현장) — "고객 등록 이후에 삭제 가능할 수 있도록 가능할까요?"
- **이전 상태**: 고객 생성·수정만 있고 삭제 기능 전무 → 잘못/테스트 등록 고객 제거 불가.
- **DB FK**: `consultation_records.customer_id`는 NOT NULL+NO ACTION(unlink·동반삭제 모두 위험), `booking_requests`/`portfolio_photos`는 nullable, 태그·메모·회원권거래는 CASCADE.
- **수정**:
  - `dbDeleteCustomer`(db.ts): 시술 기록 ≥1건이면 **삭제 차단**(`reason:'has_records'`); 없으면 예약·포트폴리오 customer_id를 NULL로 unlink(보존) 후 고객 삭제(태그·메모·회원권거래 CASCADE).
  - `removeCustomer`(customer-store): DB 성공 시에만 로컬 제거.
  - `customers/[id]`: 하단 "이 고객 삭제" + confirm 모달(기록 있으면 차단 안내+[시술 기록 보기], 없으면 삭제→`/customers` replace).
- **검증**: 라이브 DB 트랜잭션 시뮬레이션(unlink+delete·CASCADE·잔여물0 정리) + RLS 확인. tsc/lint/build EXIT 0. 상세 `docs/qa-deep-20260601.md` §7차.

## 27. 🟢 기본 커스텀 파츠 fallback (2026-06-02 사용자 QA 후속)
> 설정을 한 번도 안 연 샵은 DB customParts가 null이라 고객 화면에 파츠 미노출(네일숲 shop-001 등).
- **수정**: SSOT `src/data/default-parts.ts`(기본 8종) 신설 → parts-store·db.ts(`fetchShopPublicData`)·app-store(`DEFAULT_SHOP_SETTINGS`)가 공유. `settings.customParts ?? DEFAULT_CUSTOM_PARTS`로 null/미설정만 기본값 노출, 의도적 빈 배열은 존중. 런타임 fallback이라 DB 백필 불필요.
- **검증**: tsc/lint/build EXIT 0. 상세 `docs/qa-deep-20260601.md` §8차.

## 28. 🟢 파일럿 클라이언트 피드백 5건 + 카톡 OG (2026-06-04 — 지승호 대표 미팅)

> 출처: 파일럿샵 미팅 피드백 7건. 영준님 회신 — 데이터 구조 변경 없는 1·2·4·5·7번 + 카톡 OG는 파일럿 중 반영, 3번(가격 표시 방식)·6번(포트폴리오 폴더)은 파일럿 후. 상세 `docs/qa-deep-20260604.md`.

### 28.1 🟢 홈 화면 로고 확대 + 문구 오른쪽 (피드백 1)
- **수정**: `GreetingHeader.tsx` 컨테이너를 좌우 반전 — 로고를 왼쪽 첫 자식으로 옮기고 `h-9 w-9`→`h-16 w-16`(이니셜 `text-xl`), 문구 블록을 오른쪽으로. `flex items-center gap-3.5`.

### 28.2 🟢 사전상담 페이지 상단 로고 확대 (피드백 2)
- **수정**: `pre-consult/[shopId]/page.tsx` 상단 로고 `h-12 w-12`→`h-20 w-20` + `border-2 shadow-sm`. 손님이 링크 열 때 샵 로고가 먼저 크게 보여 신뢰감.

### 28.3 🟢 카톡 미리보기 썸네일 = 샵 로고 OG (피드백 2 확장)
- **수정**: 신규 `pre-consult/[shopId]/opengraph-image.tsx`(ImageResponse 1200×630 카드 — 로고+샵이름+안내문구) + `layout.tsx`에 `generateMetadata`. 로고 PNG/JPG fetch→data URI, SVG·실패 시 이니셜 폴백. 한글은 Google Fonts(Noto Sans KR) subset 동적 로딩(satori가 Pretendard variable woff2 미지원).
- **배포 후**: 카톡 OG 캐시 → 기존 링크는 카카오 캐시 초기화 도구로 1회 갱신 필요.

### 28.4 🟢 사전상담 합의 페이지 한/외 동시표시 (피드백 4)
- **수정**: `confirm/page.tsx`·`ConsultReview.tsx`. `useLabelMaps`→`{labels, koLabels}`(useT+useKo), `SummaryRow`/`ReviewRow`에 `valueKo` 병기(`locale!=='ko'`일 때만). 선택 항목(부위/네일/제거/길이/쉐입/랩핑/느낌/옵션/방향/키워드+카테고리)에 한국어 병기. 자유 요청사항·파츠 이름·customerNotice는 원문 유지.

### 28.5 🟢 메뉴 개수 8개 상한 해제 (피드백 5)
- **수정**: `MAX_CUSTOM_CATEGORIES` 4→20(총 24). 안내문구 "(최대 N개)"→"필요한 만큼 추가", 카운터 "N/MAX"→"N개". 상수 하나로 settings guard·렌더·문구 일괄 반영. 고객 CategoryPicker는 slice 없이 전부 렌더.

### 28.6 🟢 링크 복사 시 안내 문구 첨부 (피드백 7)
- **수정**: SSOT `src/lib/share-link-message.ts` `buildPreConsultShareMessage` 신설 → `ShareLinkCard`(홈)·`ConsultationLinkModal`(예약별)이 공유. URL만 복사하던 것을 "샵이름 사전상담 + 안내 + URL"로.

- **검증**: lint 0 error / tsc EXIT 0 / build EXIT 0. OG 라우트 생성 확인. 상세 `docs/qa-deep-20260604.md`.
