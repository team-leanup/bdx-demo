  ...
  return (
    <div ref={ref} className="fixed top-4 right-4 z-50 safe-top">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 px-3 py-2 min-h-[44px] rounded-full bg-surface border border-border shadow-sm text-xs font-bold text-text hover:border-primary/40 hover:bg-surface-alt transition-all"
